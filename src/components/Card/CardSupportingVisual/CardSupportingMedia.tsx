...
export default function CardSupportingVisual({ type, src, alt, icon, ...rest }: CardSupportingVisualType) {
    ...
    return (
        <div
            {...rest}
            className="flex h-40 w-40 shrink-0 items-center justify-center overflow-hidden rounded-full border border-black"
        >
            {type === 'image' ? (
                <img width={40} height={40} src={src} alt={alt} className="h-40 w-40 rounded-full object-cover" />
            ) : (
                icon
            )}
        </div>
