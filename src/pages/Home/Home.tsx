      ...
      {HOME_LIST.map(({ list, name }) => (
        <div key={name} className="flex w-full flex-col">
          ...
          <div className="flex flex-wrap gap-5">
            {list.map((data) => (
              <div key={data} className="flex w-full flex-col tablet:w-[calc(50%-10px)] laptop:w-[calc(33%-10.5px)]">
                <Link
                  title="컴포넌트 확인하기"
                  to={
                    name === 'Demo' ? `/demo/${data.toLowerCase()}` : `/example/${name.toLowerCase()}?component=${data}`
                  }
                >
