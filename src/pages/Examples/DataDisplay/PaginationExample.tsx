      ...
      <div className={customStyles.playgroundControlContainer}>
        ...
        >
          {PAGINATION_VARIANT.map((val) => (
            <Radio key={val} label={val} name="buttonVariant" size={isTablet ? 'small' : 'medium'} value={val} />
          ))}
        </RadioGroup>
        <strong className="text-14 tablet:text-16">
        ...
        >
          {PAGINATION_SIZE.map((val) => (
            <Radio key={val} label={val} name="buttonSize" size={isTablet ? 'small' : 'medium'} value={val} />
          ))}
        </RadioGroup>
        <strong className="text-14 tablet:text-16">
        ...
        >
          {PAGINATION_SHAPE.map((val) => (
            <Radio key={val} label={val} name="buttonShape" size={isTablet ? 'small' : 'medium'} value={val} />
          ))}
        </RadioGroup>
      </div>
